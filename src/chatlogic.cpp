...
ChatLogic::~ChatLogic()
{
    ...
    // Task 5 Responsibility moved 

    // delete all nodes
    // This should now be handled by smart pointer
    // Task 3

    //This should now be handled by smart pointers
    //Task 4
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
        ...
        while (getline(file, lineStr))
        {
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        //// STUDENT CODE Changed Task 3
                        ////

                        // check if node with this ID exists already | adapted to unique pointer
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](const std::unique_ptr<GraphNode> &node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist | adapted to unique pointer
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // get iterator on incoming and outgoing node via ID search | adapted for node representation changes Task 3
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0) {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); //adaptet to unique point | assign current node to root
            }
            else
            {
